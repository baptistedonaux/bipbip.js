...
function _gitRemoteSetBranches(config) {
	return new Promise((resolve) => {
		exec.local(`git remote set-branches --add origin ${config.repository.branch}`, {
			cwd: config.workspace,
		}).then(() => {
			resolve(config);
		});
...
function _gitShowRef(config) {
	return new Promise((resolve) => {
		exec.local(`[ -n "\`git show-ref refs/heads/${config.repository.branch}\`" ] && git checkout -q ${config.repository.branch} || git checkout -b ${config.repository.branch} origin/${config.repository.branch}`, {
			cwd: config.workspace,
		}).then(() => {
			resolve(config);
		});
...
function _gitPullBranch(config) {
	return new Promise((resolve) => {
		exec.local(`git pull origin ${config.repository.branch}`, {
			cwd: config.workspace,
		}).then(() => {
			resolve(config);
		});
...
function _gitPullTag(config) {
	return new Promise((resolve) => {
		exec.local(`git pull origin ${config.repository.tag}`, {
			cwd: config.workspace,
		}).then(() => {
			resolve(config);
		});
...
function _gitCheckoutTag(config) {
	return new Promise((resolve) => {
		exec.local(`git checkout -q refs/tags/${config.repository.tag}`, {
			cwd: config.workspace,
		}).then(() => {
			resolve(config);
		});
