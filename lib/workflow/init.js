"use strict";

const exec = require("../exec.js"),
	fs = require("fs"),
	path = require("path");

module.exports = (options) => {
	return _run(options);
};

function _run(options) {
	return new Promise((resolve) => {
		let config = options.config;

		if (config.repository === null || config.repository.url === null) {
			config.workspace = path.dirname(options.file);

			resolve(config);
		} else {
			Promise.resolve(options.config)
				.then(_git)
				.then((config) => {
					if (config.internal.gitClone === true) {
						return Promise.resolve(config)
							.then(_gitClone)
							.then(() => {
								return config;
							});
					} else {
						return Promise.resolve(config)
							.then(_gitRevParse)
							.then(_gitResetAndFetch)
							.then((config) => {
								if (config.repository.branch !== null) {
									return Promise.resolve(config)
										.then(_gitRemoteSetBranches)
										.then(_gitShowRef)
										.then(_gitPullBranch);
								} else if (config.repository.tag !== null) {
									return Promise.resolve(config)
										then(_gitCheckoutTag)
										then(_gitPullTag);
								}
							})
							.then(() => {
								return config;
							});
					}
				})
				.then((config) => {
					resolve(config);
				});
		}
	}).then((config) => {
		return config;
	});
}

function _git(config) {
	return new Promise((resolve) => {
		if (fs.existsSync(config.workspace) === true) {
			config.internal.gitClone = false;
		} else {
			config.internal.gitClone = true;
		}

		resolve(config);
	});
}

function _gitClone(config) {
	let gitOption = "";
	if (config.repository.options.submodules === true) {
		gitOption += " --recursive";
	}

	return exec.local(`git clone -b ${config.repository.branch} ${config.repository.url} --depth=1 ${config.workspace} ${gitOption}`, {
	}).then(() => {
		return config;
	});
}

function _gitRevParse(config) {
	return exec.local("git rev-parse --git-dir", {
		cwd: config.workspace,
	}).then(() => {
		return config;
	}).catch((error) => {
		fs.rmdirSync(config.workspace);
	});
}

function _gitResetAndFetch(config) {
	return exec.local([
			"git reset --hard",
			"git fetch --all",
			"git fetch --tags"
		].join(" && "), {
			cwd: config.workspace,
	}).then(() => {
		return config;
	});
}

function _gitRemoteSetBranches(config) {
	return exec.local(`git remote set-branches --add origin ${config.repository.branch}`, {
		cwd: config.workspace,
	}).then(() => {
		return config;
	});
}

function _gitShowRef(config) {
	return exec.local(`[ -n "\`git show-ref refs/heads/${config.repository.branch}\`" ] && git checkout -q ${config.repository.branch} || git checkout -b ${config.repository.branch} origin/${config.repository.branch}`, {
		cwd: config.workspace,
	}).then(() => {
		return config;
	});
}

function _gitPullBranch(config) {
	return exec.local(`git pull origin ${config.repository.branch}`, {
		cwd: config.workspace,
	}).then(() => {
		return config;
	});
}

function _gitPullTag(config) {
	return exec.local(`git pull origin ${config.repository.tag}`, {
		cwd: config.workspace,
	}).then(() => {
		return config;
	});
}

function _gitCheckoutTag(config) {
	return exec.local(`git checkout -q refs/tags/${config.repository.tag}`, {
		cwd: config.workspace,
	}).then(() => {
		return config;
	});
}
