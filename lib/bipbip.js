...
Bipbip = function(path, env) {
	...
	if (this._execLocal("whereis git") == "git:\n") {
		Bipbip._log("Missing git", "red")
		process.exit(1)
	}

	// Prepare current configuration
	var fileConfig = require(path).config,
		envConfig = Bipbip._merge(Bipbip.default, fileConfig.default, fileConfig[env])
	...
	var checkRequiredFunction = function(option, value, parents) {
		...
		if (typeof value === "object") {
			...
			if (valueTracked === undefined || valueTracked === null || valueTracked === "") {
				Bipbip._log(parents.join(".") + " is required. " + value, "red")
				process.exit(1)
			}
		}
	}
	...
	this.internal = {
		release: this._execLocal('date +"%Y%d%m%H%M%S"').split("\n")[0],
	}
};
...
Bipbip._merge = function() {
    var merge = {}

    for (var i = 0; i < arguments.length; i++) {

        for (var tmp in arguments[i]) {


			if (typeof arguments[i][tmp] === "object") {
				if (merge[tmp] === undefined) {
					merge[tmp] = arguments[i][tmp]
				} else {
					for (var item in arguments[i][tmp]) {
