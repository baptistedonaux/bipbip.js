...
const Bipbip = function(path, env, writter) {
    ...
    Promise.resolve().then(() => {
        ...
        for (let i = 0; i < options.config.requirements.local.length; i++) {
            promises.push(_local(`which ${options.config.requirements.local[i]} || exit 1`));
        }

        for (let i = 0; i < options.config.servers.length; i++) {
            promises.push(_local(`ssh -o StrictHostKeyChecking=no -o PasswordAuthentication=no -q ${options.config.servers[i].user}@${options.config.servers[i].host} 'exit'`));
        }
