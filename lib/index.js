...
Bipbip.prototype.run = function() {
	...
	this.send(
		Bipbip.prototype.shared.bind(
			this,
			Bipbip.prototype.remote.bind(
				this,
				Bipbip.prototype.deploy.bind(
					this,
					Bipbip.prototype.postDeploy.bind(
						this,
						Bipbip.prototype.clean.bind(this)
					)
				)
			)
		)
...
Bipbip.prototype.send = function(callback) {
	this._execRemotes(function(server, resolve, reject) {
		let remotePath = `${server.to}/releases/${this.internal.release}`;
		let _ = function(lastRelease) {
			exec.remote(
				server,
				helper.mkdir(
					`${server.to}/releases`,
			...
			if (lastRelease !== "") {
				...
				exec.remote(server, mkdirRemotePath);
			}
		...
		exec.remoteAsync(server, `ls -1t ${server.to}/releases | head -n 1`, _.bind(this));

	}, callback);
};

Bipbip.prototype.shared = function(callback) {
	this._execRemotes(function(server) {

		let folders = this.config.shared.folders.map(function(folder) {
			return `${server.to}/shared/${folder}`;
		});
		...
		let release = this.internal.release,
			symbolics = this.config.shared.folders.map(function(folder) {
				return helper.ln(
					`${server.to}/shared/${folder}`,
					`${server.to}/releases/${release}/${folder}`
					);
			});
		...
		for (let i = this.config.shared.files.length - 1; i >= 0; i--) {
			let pathParsed = path.parse(this.config.shared.files[i]);

			exec.remote(
				server,
				[
					helper.mkdir(`${server.to}/shared/${pathParsed.dir}`),
					helper.touch(`${server.to}/shared/${pathParsed.dir}/${pathParsed.base}`),
					helper.ln(
...
Bipbip.prototype.remote = function(callback) {
	this._execRemotes(function(server) {
	    exec.remote(
	    	server,
	    	`cd ${server.to}/releases/${this.internal.release} && ${this.config.commands.remote.join(" && ")}`
	    	);
	}, callback);
};

Bipbip.prototype.deploy = function(callback) {
	this._execRemotes(function(server) {
		exec.remote(
			server,
			`rm -f ${server.to}/current && ln -s ${server.to}/releases/${this.internal.release} ${server.to}/current`
			);
	}, callback);
};

Bipbip.prototype.postDeploy = function(callback) {
	this._execRemotes(function(server) {
	    exec.remote(
	    	server,
			`cd ${server.to}/releases/${this.internal.release} && ${this.config.commands.postDeploy.join(" && ")}`
	    	);
	}, callback);
};
...
Bipbip.prototype.clean = function() {
	this._execRemotes(function(server) {
		exec.remote(
			server,
			`ls -1dt ${server.to}/releases/* | tail -n +${(this.config.releases + 1).toString()} | xargs rm -rf`
			);
	});
};

Bipbip.prototype._execRemotes = function(fn, cb) {
	let promises = this.config.servers.map((server) => {
		return new Promise((resolve, reject) => {
			fn.call(this, server, resolve, reject);
		});
	});
