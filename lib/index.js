...
Bipbip.prototype.deploy = function() {
	...
	let _ = function(server) {
		_this._execRemote("rm -f " + server.to + "/current && ln -s " + server.to + "/releases/" + _this.internal.release + " " + server.to + "/current");
	};
	...
	let _ = function(server) {
		if (_this.config.commands.postDeploy.length > 0) {
			for (let i = 0; i < _this.config.commands.postDeploy.length; i++) {
			    _this._execRemote("cd " + server.to + "/releases/" + _this.internal.release + " && " + _this.config.commands.postDeploy[i]);
			}
		}
	};
...
Bipbip.prototype.clean = function() {
	...
	let _ = function(server) {
		_this._execRemote("ls -1dt " + server.to + "/releases/* | tail -n +" + (_this.config.releases + 1).toString() + " | xargs rm -rf");
	};
