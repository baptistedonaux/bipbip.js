...
const Bipbip = function(path, env) {
	...
	let checkRequiredFunction = function(option, value, parents) {
		...
		if (typeof value === "object") {
			...
			if (valueTracked === undefined) {
				if (parents[0] == "server" && Array.isArray(envConfig["server"])) {
					valueTracked = envConfig["server"][0][parents[1]];
				}
			}
	...
	for (let option in constant.required) {
...
Bipbip.prototype.send = function() {
	let remotePath = this.config.server.to + "/releases/" + this.internal.release;
	let lastRelease = this._execRemote("ls -1t " + this.config.server.to + "/releases | head -n 1");
