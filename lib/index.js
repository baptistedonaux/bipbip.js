...
Bipbip.prototype.shared = function() {
	...
	for (var i = this.config.shared.folders.length - 1; i >= 0; i--) {
		...
		cmds.push({
			type: "mkdir",
			path: this.config.server.to + "/shared/" + this.config.shared.folders[i]
		});
		...
		cmds.push({
			type: "ln",
			from: this.config.server.to + "/shared/" + this.config.shared.folders[i],
			to: this.config.server.to + "/releases/" + this.internal.release + "/" + this.config.shared.folders[i]
		})
	...
	for (var i = this.config.shared.files.length - 1; i >= 0; i--) {
		...
		cmds.push({
			type: "mkdir",
			path: this.config.server.to + "/shared/" + folder
		})
		...
		if (file != "/") {
			...
			cmds.push({
				type: "touch",
				path: this.config.server.to + "/shared/" + folder + file
			});
			...
			cmds.push({
				type: "ln",
				from: this.config.server.to + "/shared/" + this.config.shared.files[i],
				to: this.config.server.to + "/releases/" + this.internal.release + "/" + this.config.shared.files[i]
			});
			...
			cmds.push({
				type: "ln",
				from: this.config.server.to + "/shared/" + folder,
				to: this.config.server.to + "/releases/" + this.internal.release + "/" + folder
			});
	...
	var tpl = require("ejs").render(require("fs").readFileSync(process.cwd() + "/lib/script.sh.tpl").toString(), {
    	steps: cmds
   	});
