...
const Bipbip = function(path, env, writter) {
    ...
    Promise.resolve().then(() => {
        ...
        for (let i = 0; i < options.config.servers.length; i++) {
            promises.push(
                _local(
                    `ssh -o StrictHostKeyChecking=no -o PasswordAuthentication=no -q ${options.config.servers[i].user}@${options.config.servers[i].host} 'echo "Connection established" && exit'`,
                    {},
                    true
                )
            );
        ...
        return _local('date +"%Y%d%m%H%M%S"', {}, true)
            .then((response) => {
