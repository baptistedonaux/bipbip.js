...
module.exports.remoteAsync = function(server, command, callback, escape) {
	command =  `ssh -o StrictHostKeyChecking=no ${server.user}@${server.host} '${command}'`;
	helper.log(command, "green", server);

	let response = childProcess.exec(command, function(error, stdout, stderr) {
		let response = new Buffer(stdout).toString();
